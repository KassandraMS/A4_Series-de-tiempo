...
En una serie temporal, la estacionalidad se refiere a los patrones que se repiten a intervalos regulares, como meses, trimestres o años. Estos ciclos son producto de comportamientos periódicos (por ejemplo, aumentos de ventas en diciembre o caídas en enero).
...
Modelo (forma común de la ADF):
$$\Delta Y_t = \alpha + \beta t + \gamma Y_{t-1} + \sum_{i=1}^{p}\phi_i \, \Delta Y_{t-i} + \varepsilon_t$$
Hipótesis:
$$H_0: \gamma = 0 \quad (\text{raíz unitaria, no estacionaria}) \qquad H_1: \gamma < 0 \quad (\text{estacionaria})$$
Criterio: p-value < 0.05 → evidencia de estacionariedad.
...
La autocorrelación parcial (PACF) mide la correlación directa entre $Y_t$ y $Y_{t-k}$, eliminando los efectos de los rezagos intermedios.Ambas funciones ayudan a:
...
* $m$  → periodicidad (12 para datos mensuales).
...
Al analizar la serie original se identificó una clara tendencia creciente y presencia de estacionalidad mensual. Para eliminar dichos componentes y lograr una serie estacionaria, se aplicó una diferenciación de primer orden.
...
donde:
- \(\mathbf{x}_t\) es el vector de entradas (rezagos de la serie),
- \(\mathbf{h}_t\) es el estado oculto,
- \(\hat{y}_t\) es la salida pronosticada,
- \(f(\cdot)\) y \(g(\cdot)\) suelen ser funciones de activación no lineales.
